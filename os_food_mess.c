...
RQ =(struct Queue*) malloc(sizeof(struct Queue));
        printf("Increment Priority 0.2 per unit time\n");
	printf("Please Enter No of Persons :::");
	scanf("%d",&n);
        printf("Please Time taken to service one person :::");
        int ttt;
        scanf("%d",&ttt);
	process_all=(struct Process_Cont_struct *)malloc(sizeof(struct Process_Cont_struct)*n);
	for(i=0;i<n;i++)
	{
             printf("\n\n Enter Id no For %d Person ::: ",(i+1));
	     scanf("%d",&(process_all[i].pid));
	     printf("\n Enter arrival time For %d Person ::: ",(i+1));
	     scanf("%d",&(process_all[i].at));
            printf("\n Enter Type For %d Person [1] Student [2] Teacher ::: ",(i+1));
	     scanf("%d",&(process_all[i].type)); 
	     process_all[i].priority=process_all[i].type;
            process_all[i].exect=process_all[i].timeleft=ttt;
...
printf("\n\nGannt Chart ******\n-------------------------------------------------------------------------------------------------------------------------------------------------\n");
struct Process_Cont_struct *pr,*prev,*tpr;
while(1)
{
        UPQ();
        ...
        if(RQ->left!=NULL && DO_CT==1)
	{
             ...
             prev=pr;
	     pr=deQueue();
             if(prev!=pr && pr->type==1)
            {
               printf("%d ] Process%d |Student|[",nowtime,pr->pid);
            }
	   if(prev!=pr && pr->type==2)
            {
               printf("%d ] Process%d |Teacher|[",nowtime,pr->pid);
            }
	     ...
	     nowtime++;
             ...
             for(i=0;i<n;i++)
	     {
		if(process_all[i].state==READY&&process_all[i].timeleft>0&&process_all[i].type==1)
		{
                  
                  process_all[i].timeinwt++;
                  process_all[i].priority+=0.2;
		}
